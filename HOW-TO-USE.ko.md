...
턴이 끝나면 상태 바가 잠깐 그 턴의 비용을 보여주고 카운트다운으로 돌아가요:

```text
84k 입력 | 히트 82% | 1.3k 출력             ← 이번 턴의 토큰과 캐시 히트율
5시간 22.0% (+0.4%) | 7일 23.0%             ← 실제 구독 사용량과 이번 턴의 몫
```

두 번째 줄은 [구독 사용량](#구독-사용량-5h--7d)을 연결해야 떠요 (또는 statusline 브릿지). 어느 창이든 90% 이상이면 빨간색으로 바뀌어요.
